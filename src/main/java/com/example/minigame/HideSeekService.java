...
import com.example.minigame.job.JobTeam;
...
import com.example.minigame.audio.HideSeekAudioController;
import com.example.minigame.orchestration.HideSeekCombatAbilityOrchestrationService;
import com.example.minigame.orchestration.HideSeekDisguiseStateService;
import com.example.minigame.orchestration.HideSeekPhaseFlowOrchestrationService;
import com.example.minigame.orchestration.HideSeekTeamAssignmentService;
import com.example.minigame.stats.GameStats;
import com.example.minigame.stats.HideSeekStatsDomainService;
import com.example.minigame.stats.JobStats;
import com.example.minigame.stats.PlayerStats;
import com.example.minigame.ui.HideSeekMenuController;
import com.example.minigame.util.HideSeekLineUtil;
import com.example.minigame.util.HideSeekMathUtil;
import com.example.minigame.util.HideSeekNumberFormatUtil;
import com.example.minigame.util.HideSeekTextRenderUtil;
...
public final class HideSeekService {
    ...
    private void applySeekerRevealItemStats(ServerPlayerEntity player) {
        ...
        AttributeModifiersComponent expectedModifiers = this.createRevealItemAttributeModifiers();
        boolean changed = false;

        for (int slot = 0; slot < player.getInventory().size(); slot++) {
            ItemStack stack = player.getInventory().getStack(slot);
            if (!stack.isOf(this.revealItem)) {
                continue;
            }
            this.applyConfiguredItemPresentation(stack, this.revealItemName, this.revealItemLore);
    ...
    private void tickMagicianSpinEffects() {
        ...
        for (Map.Entry<UUID, Long> entry : new HashMap<>(this.magicianSpinUntilTickByPlayer).entrySet()) {
            ...
            String rotateCommand = "execute as " + target.getNameForScoreboard() + " at @s run tp @s ~ ~ ~ " + yaw + " " + pitch;
            this.server.getCommandManager().executeWithPrefix(this.server.getCommandSource().withSilent(), rotateCommand);
        }
    }
    ...
    public void tickGamePhase() {
        ...
        if (this.gamePhase == GamePhase.COUNTDOWN && now >= this.phaseEndTick) {
        ...
        } else if (this.gamePhase == GamePhase.HIDING && now >= this.phaseEndTick) {
        ...
        } else if (this.gamePhase == GamePhase.COMBAT && now >= this.phaseEndTick) {
            this.startWinSequence(false, this.textMessage("win_blocks_time"));
        } else if (this.gamePhase == GamePhase.ENDING && now >= this.phaseEndTick) {
            this.finishRoundState(true, true);
    ...
    private void openMainSelectionMenu(ServerPlayerEntity player) {
        this.menuController.openMainSelectionMenu(player);
    }

    private void openTeamPreferenceMenu(ServerPlayerEntity player) {
        this.menuController.openTeamPreferenceMenu(player);
    }

    private void openJobSelectionMenu(ServerPlayerEntity player) {
        this.menuController.openJobSelectionMenu(player);
    }

    private void openOpControlMenu(ServerPlayerEntity player) {
        this.menuController.openOpControlMenu(player);
    }

    private void openPersonalStatsMenu(ServerPlayerEntity player) {
        this.menuController.openPersonalStatsMenu(player);
    }
    ...
    public List<Text> menuBuildGameStatsLore() {
        return this.buildGameStatsLore();
    }

    public List<Text> menuBuildBlockStatsLore(ServerPlayerEntity player) {
        return this.buildBlockTeamStatsLore(player);
    }

    public List<Text> menuBuildSeekerStatsLore(ServerPlayerEntity player) {
        return this.buildSeekerTeamStatsLore(player);
    }

    public List<Text> menuBuildCommonStatsLore(ServerPlayerEntity player) {
        return this.buildCommonStatsLore(player);
    }

    public eu.pb4.sgui.api.elements.GuiElement menuJobButton(
            ServerPlayerEntity player,
            Item item,
            String title,
            java.util.function.Supplier<Text> action,
            java.util.function.Consumer<ServerPlayerEntity> refresh
    ) {
        return new GuiElementBuilder(item)
                .setName(this.renderMessage(title, 0, 0, "", ""))
    ...
    public eu.pb4.sgui.api.elements.GuiElement menuBackButton(ServerPlayerEntity player, java.util.function.Consumer<ServerPlayerEntity> opener) {
        return new GuiElementBuilder(Items.ARROW)
                .setName(this.guiText("common_back"))
                .setCallback((index, clickType, actionType, slotGui) -> {
                    this.playUiClickSound(player);
    ...
    public eu.pb4.sgui.api.elements.GuiElement menuCloseButton(ServerPlayerEntity player) {
        return new GuiElementBuilder(Items.BARRIER)
                .setName(this.guiText("common_close"))
                .setCallback((index, clickType, actionType, slotGui) -> {
                    this.playUiClickSound(player);
                    slotGui.close();
                })
    ...
    private List<Text> buildBlockTeamStatsLore(ServerPlayerEntity player) {
        return this.buildBlockTeamStatsLoreInternal(player);
    }

    List<Text> buildBlockTeamStatsLoreInternal(ServerPlayerEntity player) {
        PlayerStats stats = this.getOrCreatePlayerStats(player.getUuid());
        List<Text> lore = new ArrayList<>();
        lore.add(this.renderGuiStat("stats_block_games", Long.toString(stats.blockGames)));
        lore.add(this.renderGuiStat("stats_block_winrate", this.formatPercent(stats.blockWins, stats.blockGames)));

        String separator = this.textConfig.guiText("stats_job_separator");
        String blockJobRates = this.textConfig.guiText("stats_job_label_shapeshifter") + " " + this.formatPercent(stats.jobWins("block_shapeshifter"), stats.jobGames("block_shapeshifter"))
                + separator + this.textConfig.guiText("stats_job_label_attention_seed") + " " + this.formatPercent(stats.jobWins("block_attention_seed"), stats.jobGames("block_attention_seed"))
                + separator + this.textConfig.guiText("stats_job_label_magician") + " " + this.formatPercent(stats.jobWins("block_magician"), stats.jobGames("block_magician"));
        ...
        lore.add(this.renderGuiStat("stats_block_survival", this.formatAverageSeconds(stats.blockSurvivalTicks, stats.blockSurvivalRounds)));
        return lore;
    }

    private List<Text> buildSeekerTeamStatsLore(ServerPlayerEntity player) {
        return this.buildSeekerTeamStatsLoreInternal(player);
    }

    List<Text> buildSeekerTeamStatsLoreInternal(ServerPlayerEntity player) {
        PlayerStats stats = this.getOrCreatePlayerStats(player.getUuid());
        List<Text> lore = new ArrayList<>();
        lore.add(this.renderGuiStat("stats_seeker_games", Long.toString(stats.seekerGames)));
        lore.add(this.renderGuiStat("stats_seeker_winrate", this.formatPercent(stats.seekerWins, stats.seekerGames)));

        String separator = this.textConfig.guiText("stats_job_separator");
        String seekerJobRates = this.textConfig.guiText("stats_job_label_hunter") + " " + this.formatPercent(stats.jobWins("seeker_hunter"), stats.jobGames("seeker_hunter"))
                + separator + this.textConfig.guiText("stats_job_label_bomber") + " " + this.formatPercent(stats.jobWins("seeker_bomber"), stats.jobGames("seeker_bomber"))
                + separator + this.textConfig.guiText("stats_job_label_warden") + " " + this.formatPercent(stats.jobWins("seeker_warden"), stats.jobGames("seeker_warden"));
    ...
    private List<Text> buildCommonStatsLore(ServerPlayerEntity player) {
        return this.buildCommonStatsLoreInternal(player);
    }

    List<Text> buildCommonStatsLoreInternal(ServerPlayerEntity player) {
        PlayerStats stats = this.getOrCreatePlayerStats(player.getUuid());
        List<Text> lore = new ArrayList<>();
    ...
    private List<Text> buildGameStatsLore() {
        return this.buildGameStatsLoreInternal();
    }

    List<Text> buildGameStatsLoreInternal() {
        GameStats stats = this.statsDomainService.state().game;
        List<Text> lore = new ArrayList<>();
        lore.add(this.renderGuiStat("stats_avg_game_time", this.formatAverageSeconds(stats.totalCombatTicks, stats.totalGames)));
        String winrate = this.formatPercent(stats.seekerWins, stats.totalGames) + " / " + this.formatPercent(stats.blockWins, stats.totalGames);
        lore.add(this.renderGuiStat("stats_game_winrate", winrate));
        lore.add(this.renderGuiStat("stats_avg_kills", this.formatAverageDecimal(stats.totalKills, stats.totalGames)));
        lore.add(this.renderGuiStat("stats_avg_reveals", this.formatAverageDecimal(stats.totalReveals, stats.totalGames)));
    ...
    private Text renderGuiStat(String key, String value) {
        String raw = this.textConfig.guiText(key).replace("{value}", value);
        return this.renderMessage(raw, 0, 0, "", "");
    }

    private String formatPercent(long wins, long total) {
        return HideSeekNumberFormatUtil.formatPercent(wins, total);
    }

    private String formatAverageDecimal(long total, long count) {
        return HideSeekNumberFormatUtil.formatAverageDecimal(total, count);
    }

    private String formatAverageSeconds(long totalTicks, long count) {
    ...
    private void giveRoundStartItemsAndEquipment() {
        for (ServerPlayerEntity player : this.server.getPlayerManager().getPlayerList()) {
            ...
            if (this.isSeekerTeamMember(player)) {
                ...
                if (job != null) {
                    switch (job.type()) {
                        case SHAPESHIFTER -> this.ensureConfiguredItem(player, this.createJobAbilityItem(Items.SLIME_BALL));
                        case ATTENTION_SEED -> this.ensureConfiguredItem(player, this.createJobAbilityItem(Items.FIREWORK_ROCKET));
                        case MAGICIAN -> this.ensureConfiguredItem(player, this.createJobAbilityItem(Items.BLAZE_ROD));
                        default -> {
                        }
                    }
                }
    ...
    private ItemStack createJobAbilityItem(Item item) {
        ...
        this.applyConfiguredItemPresentation(stack, this.resolveJobAbilityItemName(item), this.resolveJobAbilityItemLore(item));
        return stack;
    }
        ...
        for (int slot = 0; slot < player.getInventory().size(); slot++) {
            ...
            if (stack.isOf(item)) {
                if (item == this.revealItem) {
                    this.applyConfiguredItemPresentation(stack, this.revealItemName, this.revealItemLore);
                } else if (item == this.undisguiseItem) {
                    this.applyConfiguredItemPresentation(stack, this.undisguiseItemName, this.undisguiseItemLore);
                } else {
                    this.applyConfiguredItemPresentation(stack, this.resolveJobAbilityItemName(item), this.resolveJobAbilityItemLore(item));
                }
                return;
    ...
    private ItemStack createConfiguredRevealItem() {
        ItemStack stack = new ItemStack(this.revealItem);
        this.applyConfiguredItemPresentation(stack, this.revealItemName, this.revealItemLore);
        stack.set(DataComponentTypes.ATTRIBUTE_MODIFIERS, this.createRevealItemAttributeModifiers());
        return stack;
    }

    private ItemStack createConfiguredUndisguiseItem() {
        ItemStack stack = new ItemStack(this.undisguiseItem);
        this.applyConfiguredItemPresentation(stack, this.undisguiseItemName, this.undisguiseItemLore);
        return stack;
    }

    private void applyConfiguredItemPresentation(ItemStack stack, String name, List<String> lore) {
        if (name != null && !name.isBlank()) {
            stack.set(DataComponentTypes.CUSTOM_NAME, this.withoutItalic(this.renderMessage(name, 0, 0, "", "")));
        }
        ...
        for (String line : lore) {
            ...
            Text loreText = this.withoutItalic(this.renderMessage(line, 0, 0, "", ""));
    ...
    private String resolveJobAbilityItemName(Item item) {
        ...
        if (item == Items.RECOVERY_COMPASS) {
            return this.jobConfig.wardenItemName();
        }
        if (item == Items.SLIME_BALL) {
            return this.jobConfig.shapeshifterItemName();
        }
        if (item == Items.FIREWORK_ROCKET) {
            return this.jobConfig.attentionSeedItemName();
        }
        if (item == Items.BLAZE_ROD) {
            return this.jobConfig.magicianItemName();
        }
        return "";
    }
    ...
    private List<String> resolveJobAbilityItemLore(Item item) {
        ...
        if (item == Items.RECOVERY_COMPASS) {
            return this.jobConfig.wardenItemLore();
        }
        if (item == Items.SLIME_BALL) {
            return this.jobConfig.shapeshifterItemLore();
        }
        if (item == Items.FIREWORK_ROCKET) {
            return this.jobConfig.attentionSeedItemLore();
        }
        if (item == Items.BLAZE_ROD) {
            return this.jobConfig.magicianItemLore();
        }
        return List.of();
    }
    ...
    private void teleportAllToSpawn() {
        ...
        String x = Double.toString(this.spawnX);
        String y = Double.toString(this.spawnY);
        String z = Double.toString(this.spawnZ);

        for (ServerPlayerEntity player : this.server.getPlayerManager().getPlayerList()) {
            String command = "execute in " + worldId + " run tp " + player.getNameForScoreboard() + " " + x + " " + y + " " + z;
    ...
    private void teleportTeamToArena(boolean seekerTeam) {
        ...
        String x = Double.toString(this.arenaX);
        String y = Double.toString(this.arenaY);
        String z = Double.toString(this.arenaZ);
        for (ServerPlayerEntity player : this.server.getPlayerManager().getPlayerList()) {
            if (player.isSpectator()) {
                continue;
            }
    ...
    private void teleportSeekersToWaitingArea() {
        ...
        String x = Double.toString(this.seekerWaitingX);
        String y = Double.toString(this.seekerWaitingY);
        String z = Double.toString(this.seekerWaitingZ);
        for (ServerPlayerEntity player : this.server.getPlayerManager().getPlayerList()) {
    ...
    private void setTickRate(int rate) {
        ...
        this.server.getCommandManager().executeWithPrefix(this.server.getCommandSource().withSilent(), "tick rate " + rate);
    }
    ...
    private Item resolveRevealItem(String itemIdText) {
        ...
        if (itemId == null || !Registries.ITEM.containsId(itemId)) {
            this.logger.warn("[{}] 잘못된 reveal_item 설정: {}. minecraft:brush 사용", HideSeek.MOD_ID, itemIdText);
            return Items.BRUSH;
        }

        Item item = Registries.ITEM.get(itemId);
        if (item == Items.AIR) {
            this.logger.warn("[{}] reveal_item은 공기 아이템 사용 불가: {}. minecraft:brush 사용", HideSeek.MOD_ID, itemIdText);
            return Items.BRUSH;
        }
        return item;
    }
    ...
    private Item resolveUndisguiseItem(String itemIdText) {
        Identifier itemId = Identifier.tryParse(itemIdText);
        if (itemId == null || !Registries.ITEM.containsId(itemId)) {
            this.logger.warn("[{}] 잘못된 undisguise_item 설정: {}. minecraft:magma_cream 사용", HideSeek.MOD_ID, itemIdText);
            return Items.MAGMA_CREAM;
        }

        Item item = Registries.ITEM.get(itemId);
        if (item == Items.AIR) {
            this.logger.warn("[{}] undisguise_item은 공기 아이템 사용 불가: {}. minecraft:magma_cream 사용", HideSeek.MOD_ID, itemIdText);
            return Items.MAGMA_CREAM;
        }
        return item;
    }
